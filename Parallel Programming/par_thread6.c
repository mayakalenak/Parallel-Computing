...
int main()
{
    ...
    data[0].start = 0;
    data[0].end = N / NUMTHREADS;

    data[1].start = N / NUMTHREADS;
    data[1].end = 2 * (N / NUMTHREADS);

    data[2].start = 2 * (N / NUMTHREADS);
    data[2].end = 3 * (N / NUMTHREADS);

    data[3].start = 3 * (N / NUMTHREADS);
    data[3].end = 4 * (N / NUMTHREADS);

    data[4].start = 4 * (N / NUMTHREADS);
    data[4].end = 5 * (N / NUMTHREADS);

    data[5].start = 5 * (N / NUMTHREADS);
    data[5].end = 6 * (N / NUMTHREADS);

    data[6].start = 6 * (N / NUMTHREADS);
    data[6].end = 7 * (N / NUMTHREADS);

    data[7].start = 7 * (N / NUMTHREADS);
    data[7].end = N;
